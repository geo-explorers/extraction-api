...
from src.config.prompts.claim_extraction_prompt import CLAIM_EXTRACTION_PROMPT
...
from src.infrastructure.rate_limiter import get_gemini_rate_limiter
...
class ClaimWithTopicBaseResult(BaseModel):
    ...
    topic: str = Field(description="The topic label associated with the claim")
...
class PremiumClaimExtractor:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized PremiumClaimExtractor with model {self.model_name} (structured outputs)"
        )

    async def extract_topics_of_discussion_from_episode(
        self, title: str, description: str, full_transcript: str
    ) -> List[str]:
        """
        ...
        try:
            ...
            prompt = TOPICS_OF_DISCUSSION_PROMPT.format(
                title=title, description=description, transcript=full_transcript
            )
            logger.info(
                f"Calling {self.model_name} for topics of discussion extraction "
            )

            await get_gemini_rate_limiter().acquire()
            ...
            result: TopicDiscussionResult = TopicDiscussionResult.model_validate_json(
                response.text
            )
            topics = result.topics

            logger.info(
                f"Extracted {len(topics)} topics from full transcript via structured outputs"
            )
            return topics
    ...
    async def extract_claims_with_topics_from_transcript(
        self, full_transcript: str, topics_of_discussion: List[str]
    ) -> Dict[str, List[str]]:
        """
        ...
        try:
            prompt = CLAIM_EXTRACTION_PROMPT.format(
                transcript=full_transcript, topics_of_discussion=topics_of_discussion
            )
            ...
            result: ClaimWithTopicResult = ClaimWithTopicResult.model_validate_json(
                response.text
            )
            ...
            logger.info(
                f"Extracted {len(claims_with_topics)} claims from full transcript via structured outputs"
            )
            return parsed_result
        except Exception as e:
            logger.error(
                f"Error in premium claim extraction with topics: {e}", exc_info=True
            )
            if response:
                logger.error(f"Response text: {getattr(response, 'text', 'N/A')[:500]}")
            return {}
    ...
    async def extract_key_takeaways_from_claims(
        ...
        try:
            ...
            prompt = KEY_TAKEAWAYS_PROMPT.format(topics_with_claims=topics_with_claims)
            ...
            # Parse structured response using Pydantic
            result: KeyTakeawayResult = KeyTakeawayResult.model_validate_json(
                response.text
            )
            key_takeaways = result.key_takeaways

            logger.info(
                f"Extracted {len(key_takeaways)} key takeaways via structured outputs"
            )
            return key_takeaways

        except Exception as e:
            logger.error(
                f"Error in premium key takeaway extraction: {e}", exc_info=True
            )
            if response:
                logger.error(f"Response text: {getattr(response, 'text', 'N/A')[:500]}")
            return []
